...
export interface RailwayLog {
  ...
  message: string;
  severity?: string;
  tags?: { serviceId?: string; deploymentId?: string };
}
...
const HTTP_RE =
  /^(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+(\S+)\s+(\d{3})\s+([\d.]+)\s*ms/;
...
function LevelBadge({ severity }: { severity: string }) {
...
export function RailwayLogsDisplay({ data }: Props) {
  if (!data?.length) {
    ...
    <div className="overflow-x-auto rounded-md bg-muted/40 p-2">
      ...
      {data.length >= 50 && (
        <p className="mt-2 text-[10px] text-muted-foreground">
          Showing {data.length} logs · request more if needed
        </p>
      )}
    </div>
  );
